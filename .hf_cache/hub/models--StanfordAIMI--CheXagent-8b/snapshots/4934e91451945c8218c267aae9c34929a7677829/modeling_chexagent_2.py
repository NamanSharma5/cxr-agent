...
class CheXagentForConditionalGeneration(CheXagentPreTrainedModel):
    ...
    def generate(
            ...
            attention_mask: Optional[torch.LongTensor] = None,
            return_vit_outputs = False,
            return_qformer_outputs = False,
            generate_written_output = False,
            **generate_kwargs,
    ) -> torch.LongTensor:
        """
        if all of `return_vit_outputs`, `return_qformer_outputs`, and `generate_written_output` are false, run CheXagent as norma
        if `return_vit_outputs` or `return_qformer_outputs` is true and not `generate_written_output`, then only return embeddings
        if `generate_written_output` is true and one of `return_vit_outputs` or `return_qformer_outputs`, only generate from text prompt
        if `generate_written_output` is true and not `return_vit_outputs` or `return_qformer_outputs`, only generate from text prompt
        """
        if hasattr(self, "hf_device_map"):
            # preprocess for `accelerate`
            self._preprocess_accelerate()

        if return_qformer_outputs and return_vit_outputs:
            return RuntimeError("Only one of `return_vit_outputs` and `return_qformer_outputs` can be true")

        if generate_written_output and not (return_vit_outputs or return_qformer_outputs):
            ## simpler to reason about, but possibly can make cleaner 
            inputs_lang = self.language_model.get_input_embeddings()(input_ids)
            lang_atts = attention_mask
            if lang_atts is None:
                lang_atts = torch.ones_like(input_ids)
        ...
        if pixel_values is not None:
            ...
            if return_vit_outputs and not generate_written_output:
            ...
            # step 3: project vision to language
            if return_qformer_outputs and not generate_written_output:
                return query_output
        ...
        if return_vit_outputs and generate_written_output:
            return vision_outputs.pooler_output, outputs
        
        if return_qformer_outputs and generate_written_output:
            return query_output, outputs
